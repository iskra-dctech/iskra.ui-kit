...
Документ описывает продуктовый контекст, правила контента и визуальные принципы дизайн-системы **Искра.DCI**. Техническая реализация токенов и компонентов — в пакетах `@iskra-dci/*` (см. [PACKAGES.md](./PACKAGES.md)).
...
Токены определены в `@iskra-dci/tokens` и применяются через `@iskra-dci/styles`.

### Типографика

Два семейства, **self-hosted** (без CDN), поставляются пакетом `@iskra-dci/styles`:
...
В монорепозитории канонический набор — пакет **`@iskra-dci/icons`**: 16×16, stroke 1.5px, framework-agnostic. React-компонент `Icon` из `@iskra-dci/react` использует этот набор.
