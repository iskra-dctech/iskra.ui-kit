...
**Критическое правило:** mint не несёт текст на тёмном (контраст ≈1.6:1, WCAG fail) — для акцентного *текста* используйте AA-безопасный `#00AA85` (`--accent-safe` в токенах).
