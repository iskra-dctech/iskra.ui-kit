...
describe('computeAnchorPosition', () => {
  it('places panel below anchor start', () => {
    ...
    const pos = computeAnchorPosition(anchor, { width: 80, height: 50 }, 'bottom-start', 8, viewport);
    expect(pos).toEqual({ top: 48, left: 20 });
  });

  it('clamps panel inside viewport', () => {
    const anchor = { top: 280, left: 380, right: 400, bottom: 295, width: 20, height: 15 } as DOMRect;
    const pos = computeAnchorPosition(anchor, { width: 100, height: 80 }, 'bottom-start', 8, viewport);
