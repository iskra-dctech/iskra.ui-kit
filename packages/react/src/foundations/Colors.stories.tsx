  ...
  render: () => (
    <div style={{ maxWidth: 700, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
        ...
        <TokenSwatch name="Mint Safe (text)" hint="--accent-safe · AA on dark" token="--accent-safe" />
      </div>
      <div
        style={{
          display: 'flex',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 9, maxWidth: 700 }}>
      {STATUS_ROWS.map((row) => (
        <div key={row.token} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          />
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--fg1)', width: 150 }}>{row.name}</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg2)', width: 100 }}>
            {row.token}
          </span>
